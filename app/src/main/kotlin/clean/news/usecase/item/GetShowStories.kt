...
class GetShowStories @Inject constructor(
	...
	override fun execute(flags: Int): Observable<List<Item>> {
		...
		if (strategy.memory) observables.concatWith(memory.getShowStories().onErrorResumeNext(Observable.empty()))
		if (strategy.disk) observables.concatWith(disk.getShowStories().onErrorResumeNext(Observable.empty()))
		if (strategy.network) observables.concatWith(network.getShowStories().doOnNext(save))
