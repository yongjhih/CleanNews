...
class GetAskStories @Inject constructor(
	...
	override fun execute(flags: Int): Observable<List<Item>> {
		...
		if (strategy.memory) observables.concatWith(memory.getAskStories().onErrorResumeNext(Observable.empty()))
		if (strategy.disk) observables.concatWith(disk.getAskStories().onErrorResumeNext(Observable.empty()))
		if (strategy.network) observables.concatWith(network.getAskStories().doOnNext(save))
